...
class game:
    def __init__(self, gameid, pbp=True, moments=False):
        ...
        if moments:
            with open("GameFiles/" + str(self.gameid) + ".json") as f:
                jsonf = json.load(f)
                self.gamedate = datetime.datetime.strptime(jsonf['gamedate'], '%Y-%m-%d')
                self.gamedate2 = str(self.gamedate)
                self.gameDate = self.gamedate2[0:10]
                self.gameDate = self.gameDate.replace("-", ".")
        ...
        self.hTeam = str(self.home.teamname_abbrev)
        self.aTeam = str(self.visitor.teamname_abbrev)

        if (self.hTeam == "PHX"):
            self.hTeam = self.hTeam.replace("PHX", "PHO")

        if (self.aTeam == "PHX"):
            self.aTeam = self.aTeam.replace("PHX", "PHO")

        if (self.hTeam == "BKN"):
            self.hTeam = self.hTeam.replace("BKN", "BRK")

        if (self.aTeam == "BKN"):
            self.aTeam = self.aTeam.replace("BKN", "BRK")

        if (self.hTeam == "CHA"):
            self.hTeam = self.hTeam.replace("CHA", "CHO")

        if (self.aTeam == "CHA"):
            self.aTeam = self.aTeam.replace("CHA", "CHO")

        if pbp:
            with open("PBP/" + self.gameDate + "." + self.aTeam + ".at." + self.hTeam + "." + str(self.gameid) + ".csv") as f:
...
def insertGame(gameid):
    with open("GameFiles/" + gameid + ".json") as f:
        jsonf = json.load(f)
        date = jsonf["gamedate"]
        home = team(jsonf['events'][0]['home'], True).teamname
        away = team(jsonf['events'][0]['visitor'], False).teamname
        new_game = {"game id": gameid, "game date": date, "home team": home, "away team": away}

    with open('team-Info.json', 'r') as g:
        gamef = json.load(g)
        print(gamef)
        length = len(gamef)
        i = 0
        while i < length:
            if gamef[i]["game id"] == gameid:
                print("Game already added.")
                return
            i += 1
        gamef.append(new_game)
        print(gamef)

        with open('team-Info.json', 'w') as g:
            json.dump(gamef, g)
...
if __name__ == "__main__":
    main(sys.argv[1:])
    g1 = game("0021500488", True, True)

    insertGame("0021500489")
    deleteGame("0021500489")
    print(getPassword())
    exit(0)
